...
Поток может безопасно работать со своими регистрами и стеком. Но для работы с общими ячейками памяти требуется защита от гонки данных ```data race``` и неконсистентности из-за состояний гонки ```race condition```.
