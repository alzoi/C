...
Префикс LOCK даёт сигнал процессору, что текущий поток имеет монопольный доступ к памяти. Пока выполняется данная инструкция обмена, другие потоки не могут кэшировать указанную ячейку памяти. Необходимо учитывать, что для других потоков будет заблокирована вся кэш-линия (64 байта), начиная с указаной ячейки памяти.
