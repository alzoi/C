...
Префикс LOCK даёт сигнал процессору, что текущий поток имеет монопольный доступ к памяти. Пока выполняется данная инструкция обмена, другие потоки не могут кэшировать указанную ячейку памяти. Необходимо учитывать, что для других потоков будет заблокированы все 64 байта (кэш-линия для кэша первого уровня всех ядер процессора), начиная с адреса указаной ячейки памяти.
