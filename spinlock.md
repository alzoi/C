...
Префикс LOCK даёт сигнал процессору, что текущий поток имеет монопольный доступ к памяти. Пока выполняется данная инструкция обмена, другие потоки не могут кэшировать данные из оперативной памяти размером в кэш-линию в которой находится указанная для обмена ячейка памяти.
