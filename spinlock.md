...
На практике нужно сначала крутиться в цикле получения блокировки с использованием инструкции pause. Потом, адаптивно, через некоторое время вызывать функцию yield, уступая процессорное время другим потокам. Так как функция yield делает системный вызов sched_yield (который говорит планировщику, что текущий поток можно не выполнят, а переставить в конец очереди), то эта функция будет работать медленее инструкции просессора pause.
```cpp
#include <thread>
