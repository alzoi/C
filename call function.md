...
В вызываемую функцию ```(callee)``` вызывающая функция ```(caller)``` может передать шесть аргументов через следующие регистры: ```rdi, rsi, rdx, rcx, r8, r9```. Седьмой и далее аргументы вызываемой функции передаются через стек вызывающей функции.
