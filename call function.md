...
В вызываемую функцию (callee) вызывающая функция (caller) может передать через регистры (```rdi, rsi, rdx, rcx, r8, r9```) шесть аргументов.  
Седьмой и далее аргументы вызываемой функции передаются через стек.
